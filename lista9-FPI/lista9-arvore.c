...
void insertNode(TreeNodePtr *treePtr, int value, TreeNodePtr *raizGlobal);
...
void freeArvore(TreeNodePtr treePtr);
...
int main(){
    ...
    while(num != -1){
        insertNode(&raiz, num, &raiz); // inserir os nós
    ...
    freeArvore(raiz); // libera memoria da arvore
    raiz = NULL; // vi que é bom fazer isso por segurança

    return 0;
}

void insertNode(TreeNodePtr *treePtr, int num, TreeNodePtr *raizGlobal){
    if (*treePtr == NULL){ // arvore vazia, aqui vai ocorrer a inserção
        *treePtr = malloc(sizeof(TreeNode)); // alocacar memoria
        if (*treePtr){ // alocacao ok
            ...
            freeArvore(*raizGlobal); // libera a memoria alocada ate aqui
            *raizGlobal = NULL;
            exit(1);
        }
    } else { // arvore nao vazia: vefificacao e recursao
        if (num < (*treePtr)->num){ // inserir a esquerda
            insertNode(&((*treePtr)->esquerda), num, raizGlobal);
    
        }else if (num > (*treePtr)->num){ // inserir a direita
            insertNode(&((*treePtr)->direita), num, raizGlobal);
        } // ignora os iguais
    }
}
...
void freeArvore(TreeNodePtr treePtr){
    if (treePtr != NULL){
        freeArvore(treePtr->esquerda); // subarvore esquerda
        freeArvore(treePtr->direita); //subarvore direita
        free(treePtr); // libera a raiz das subarvores e no final a raiz da arvore completa
    }
}